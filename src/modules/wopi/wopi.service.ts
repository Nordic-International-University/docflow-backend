  ...
  private async reconvertAndMergeXfdf(
    ...
    try {
      ...
      const versionCount = await this.#_prisma.attachment.count({
        where: { documentId, mimeType: 'application/pdf', isAutoGenerated: true, deletedAt: null },
      })
      ...
      const nameWithoutExt = cleanPdfName.substring(0, cleanPdfName.lastIndexOf('.'))
      ...
      await this.#_minio.putFile(uploadedPdfFileName, finalPdfBuffer, 'application/pdf')
