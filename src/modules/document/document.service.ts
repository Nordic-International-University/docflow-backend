...
export class DocumentService {
  ...
  async documentHistory(payload: {
    ...
    if (!isAdmin && document.createdById !== payload.userId) {
      ...
      if (!hasAccess) {
        throw new ForbiddenException('Bu hujjat tarixini ko\'rish huquqingiz yo\'q')
      }
    }
    ...
    for (const att of allAttachments) {
      timeline.push({
        ...
        title: att.isAutoGenerated ? 'Avtomatik PDF yaratildi' : 'Fayl yuklandi',
      ...
      for (const step of wf.workflowSteps) {
        ...
        for (const action of step.actions) {
          timeline.push({
            ...
            title: this.getActionTitle(action.actionType, step.order, step.actionType),
        ...
        if (step.completedAt && !step.isRejected) {
          timeline.push({
            ...
            meta: { stepId: step.id, order: step.order, actionType: step.actionType },
          })
        }
      }
    }
    ...
    return {
      ...
      summary: {
        ...
        totalSteps: workflows.reduce(
          (sum, wf) => sum + wf.workflowSteps.filter((s) => !s.isCreator).length,
          0,
        ),
        totalActions: workflows.reduce(
          (sum, wf) => sum + wf.workflowSteps.reduce((s, step) => s + step.actions.length, 0),
          0,
        ),
  ...
  private getActionTitle(actionType: string, stepOrder: number, stepActionType: string): string {
  ...
  async documentUpdateXfdfUrl(
    ...
    try {
      ...
      const nameWithoutExt = cleanMergedName.substring(0, cleanMergedName.lastIndexOf('.'))
      ...
      if (sanitizedMergedFileName.length > 100) {
        const extPart = sanitizedMergedFileName.substring(sanitizedMergedFileName.lastIndexOf('.'))
        sanitizedMergedFileName = sanitizedMergedFileName.substring(0, 100 - extPart.length) + extPart
      }
  ...
  async documentCreateWithOffice(payload: {
    ...
    const fileName = `${payload.title.replace(/[^a-zA-Z0-9\s-]/g, '').trim().replace(/\s+/g, '-')}.${fileType}`
  ...
  private createBlankDocx(): Buffer {
    ...
    zip.file('word/document.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t></w:t></w:r></w:p></w:body></w:document>')
  ...
  private createBlankXlsx(): Buffer {
    ...
    zip.file('xl/worksheets/sheet1.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData/></worksheet>')
    return zip.generate({ type: 'nodebuffer' })
  }
  ...
  private createBlankPptx(): Buffer {
    ...
    zip.file('ppt/presentation.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><p:presentation xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><p:sldIdLst><p:sldId id="256" r:id="rId2"/></p:sldIdLst></p:presentation>')
    ...
    zip.file('ppt/slides/slide1.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:cSld><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/><p:cNvGrpSpPr/><p:nvPr/></p:nvGrpSpPr><p:grpSpPr/></p:spTree></p:cSld></p:sld>')
    return zip.generate({ type: 'nodebuffer' })
  }
}
