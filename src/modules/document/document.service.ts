...
export class DocumentService {
   ...
   */
  /**
   * Hujjat ish jarayonini boshlash:
   * 1. DOCX → PDF qotirish (final freeze)
   * 2. DOCX'ni lock qilish (workflow active bo'lganda WOPI permission tekshiradi)
   * 3. Workflow yaratish
   * 4. Status PENDING ga o'tkazish
   */
  async documentStartWorkflow(payload: {
    documentId: string
    userId: string
    ...
    if (document.createdById !== payload.userId) {
      throw new ForbiddenException(
        "Faqat hujjat yaratuvchisi ish jarayonini boshlay oladi",
      )
    }
    if (document.status !== DocumentStatus.DRAFT) {
      throw new BadRequestException(
        'Faqat DRAFT holatdagi hujjatlarda ish jarayonini boshlash mumkin',
      )
    }

    // Mavjud workflow tekshiruvi
    const existingWorkflow = await this.#_prisma.workflow.findFirst({
      where: { documentId: payload.documentId, deletedAt: null },
    })
    if (existingWorkflow) {
      throw new ConflictException("Bu hujjat uchun ish jarayoni allaqachon mavjud")
    }

    // 2. Office faylni topish va PDF ga konvertatsiya
    ...
    if (officeAttachment) {
      try {
        ...
        throw new BadRequestException(
          "DOCX dan PDF ga aylantirib bo'lmadi: " + (err as Error).message,
        )
      }
    ...
    const firstStepOrder =
      payload.steps && payload.steps.length > 0
        ? Math.min(...payload.steps.map((s) => s.order))
        : 1

    const workflow = await this.#_prisma.$transaction(async (tx) => {
      const wf = await tx.workflow.create({
        data: {
          documentId: payload.documentId,
          currentStepOrder: firstStepOrder,
          status: 'ACTIVE',
          type: payload.type || 'CONSECUTIVE',
      ...
      if (payload.workflowTemplateId) {
        const template = await tx.workflowTemplate.findFirst({
          where: { id: payload.workflowTemplateId, deletedAt: null, isActive: true },
          include: { steps: { where: { deletedAt: null }, orderBy: { order: 'asc' } } },
        })
        if (!template || template.steps.length === 0) {
          throw new NotFoundException("Ish jarayoni shabloni topilmadi yoki bo'sh")
        }
        const isParallel = (payload.type || template.type) === 'PARALLEL'
        for (const step of template.steps) {
          const dueDate = step.dueInDays
            ? new Date(Date.now() + step.dueInDays * 86400000)
            : null
          await tx.workflowStep.create({
            data: {
              workflowId: wf.id,
              ...
              dueDate,
              isRejected: false,
              status: isParallel || step.order === firstStepOrder ? 'IN_PROGRESS' : 'NOT_STARTED',
              startedAt: isParallel || step.order === firstStepOrder ? new Date() : null,
              isCreator: false,
            },
          })
        }
      } else if (payload.steps && payload.steps.length > 0) {
        const isParallel = payload.type === 'PARALLEL'
        ...
        throw new BadRequestException("Workflow shablon yoki bosqichlar berilishi kerak")
      }
